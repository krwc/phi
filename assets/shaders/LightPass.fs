...
#define PCF_IMPL(R, Size)                                               \
    ...
    for (float i = -R + Offset.x; i <= R + Offset.x; i += R * S) {      \
        for (float j = -R + Offset.y; j <= R + Offset.y; j += R * S) {  \
            float Closest =                                             \
                    texture(g_TexShadow,                                \
                            ProjCoords + g_DepthTexelSize * vec2(i, j)) \
                            .r;                                         \
            I += CurrentDepth > Closest ? 1.0 : 0.0;                    \
        }                                                               \
    }                                                                   \
    return I / (Size * Size);
...
void main() {
    ...
    vec3 I = (1.0f - ShadowIntensity(g_ShadowMatrix * P)) * DirLightI
             + PointLightI;
    vec3 Color = clamp(I * DiffuseColor.xyz, 0, 1);
