...
Sampler::Sampler(GLenum min_filter,
        ...
        : m_bind(GL_NONE) {
    CheckedCall(phi::glCreateSamplers, 1, &m_bind);
    CheckedCall(phi::glSamplerParameteri, m_bind, GL_TEXTURE_MIN_FILTER, min_filter);
    CheckedCall(phi::glSamplerParameteri, m_bind, GL_TEXTURE_MAG_FILTER, mag_filter);
    if (type == phi::TextureType::Texture2D) {
        CheckedCall(phi::glSamplerParameteri, m_bind, GL_TEXTURE_WRAP_S, (GLenum) mode);
        CheckedCall(phi::glSamplerParameteri, m_bind, GL_TEXTURE_WRAP_T, (GLenum) mode);
    } else {
...
Sampler::~Sampler() {
    CheckedCall(phi::glDeleteSamplers, 1, &m_bind);
}
